...
| Windows x86_64 | Native Windows/MSVC Rust and MinGW OpenConnect builds exercised. The actual CLI package passed installation, all seven protocols, healthy doctor/running-service checks and idle uninstall on a GitHub-hosted runner. Publication also requires the GUI package's native installation lifecycle gate. Wintun/NRPT networking, desktop and tunnel acceptance are not inferred from those idle checks. |
